...
#error pico_network.c is firmware-only
#endif
...
static struct tcp_pcb *connection(const pbns_pico_network *network) {
  return network->connection;
}

static pbns_status get_asynchronous_status(const pbns_pico_network *network) {
...
static err_t tcp_received(void *context, struct tcp_pcb *pcb,
                          struct pbuf *packet, err_t error) {
  pbns_pico_network *const network = context;
  ...
  if (network->diagnostic != NULL &&
      pbns_raw_diagnostic_observe_tcp_rx(network->diagnostic) != PBNS_OK) {
...
#endif
  const size_t available = pbns_byte_ring_capacity(&network->tcp_rx) -
                           pbns_byte_ring_size(&network->tcp_rx);
  if ((size_t)packet->tot_len > available) {
    return ERR_MEM;
  }

  const pbns_byte_ring ring_before = network->tcp_rx;
  u16_t copied = 0U;
  while (copied < packet->tot_len) {
    pbns_buffer writable = {0};
    if (pbns_byte_ring_writable(&network->tcp_rx, &writable) != PBNS_OK ||
        writable.cap == 0U) {
      network->tcp_rx = ring_before;
      network->asynchronous_status = PBNS_ERR_STATE;
      (void)pbuf_free(packet);
      network->connection = NULL;
      tcp_abort(pcb);
    ...
    if (pbuf_copy_partial(packet, writable.ptr, amount_u16, copied) !=
            amount_u16 ||
        pbns_byte_ring_commit(&network->tcp_rx, amount) != PBNS_OK) {
      network->tcp_rx = ring_before;
      network->asynchronous_status = PBNS_ERR_STATE;
      (void)pbuf_free(packet);
      network->connection = NULL;
...
static pbns_status start_tcp_connection(pbns_pico_network *network,
...
static pbns_status tcp_read(void *context, pbns_buffer destination,
                            size_t *received) {
  pbns_pico_network *const network = context;
  if (received == NULL) {
  ...
  *received = 0U;
  ...
  pbns_status status = pbns_byte_ring_readable(&network->tcp_rx, &readable);
  if (status == PBNS_OK && readable.len > 0U) {
    ...
    status = pbns_byte_ring_consume(&network->tcp_rx, amount);
    if (status == PBNS_OK) {
...
static pbns_tcp_io_result tcp_io_result(err_t status) {
  if (status == ERR_OK) {
    return PBNS_TCP_IO_OK;
  }
  return status == ERR_MEM ? PBNS_TCP_IO_RETRY : PBNS_TCP_IO_FAILED;
}

static pbns_status tcp_write_endpoint(void *context, pbns_view source,
                                      size_t *written) {
  pbns_pico_network *const network = context;
  if (written == NULL) {
    return PBNS_ERR_ARGUMENT;
  ...
  *written = 0U;
  ...
  const pbns_tcp_io_result write_result = tcp_io_result(
      tcp_write(pcb, source.ptr, (u16_t)amount, TCP_WRITE_FLAG_COPY));
  pbns_tcp_io_result output_result = PBNS_TCP_IO_NOT_RUN;
  if (write_result == PBNS_TCP_IO_OK) {
    output_result = tcp_io_result(tcp_output(pcb));
  }
  if (write_result == PBNS_TCP_IO_FAILED ||
      (write_result == PBNS_TCP_IO_OK && output_result != PBNS_TCP_IO_OK)) {
    network->asynchronous_status = PBNS_ERR_TRANSPORT;
  }
#if defined(PBNS_RAW_TUNNEL_DIAGNOSTIC)
  if (network->diagnostic != NULL &&
      pbns_raw_diagnostic_observe_tcp_io(network->diagnostic, write_result,
                                         output_result) != PBNS_OK) {
    network->asynchronous_status = PBNS_ERR_STATE;
  }
#endif
  pbns_status status =
      pbns_tcp_write_outcome(write_result, output_result, amount, written);
#if defined(PBNS_RAW_TUNNEL_DIAGNOSTIC)
  if (network->asynchronous_status == PBNS_ERR_STATE) {
...
static pbns_status pico_wifi_start(void *context) {
  ...
  pbns_byte_ring_reset(&network->tcp_rx);
...
static pbns_status pico_wifi_poll(void *context) {
  const pbns_pico_network *const network = context;
  if (network == NULL || !network->initialized) {
    return PBNS_ERR_STATE;
  }
  cyw43_arch_lwip_begin();
...
static pbns_status pico_session_poll(void *context) {
  pbns_pico_network *const network = context;
  if (network == NULL || !network->initialized) {
    return PBNS_ERR_STATE;
  }
  ...
  cyw43_arch_lwip_end();
  return tcp_is_ready ? PBNS_OK : PBNS_ERR_WOULD_BLOCK;
}
...
pbns_status pbns_pico_network_init(pbns_pico_network *network,
  ...
  pbns_byte_ring_init(&network->tcp_rx,
                      (pbns_buffer){network->tcp_rx_storage, 0U,
                                    sizeof(network->tcp_rx_storage)});
  if (pbns_byte_ring_capacity(&network->tcp_rx) == 0U) {
  ...
  const int pm_status = cyw43_wifi_pm(&cyw43_state, CYW43_NONE_PM);
  cyw43_arch_lwip_end();
  if (pm_status != 0) {
    cyw43_arch_deinit();
    secure_zero(network, sizeof(*network));
...
pbns_network_ops pbns_pico_network_operations(pbns_pico_network *network) {
  return (pbns_network_ops){
      ...
      .session_poll = pico_session_poll,
      .close = pico_close,
...
pbns_pump_endpoint pbns_pico_network_tcp_endpoint(pbns_pico_network *network) {
  return network == NULL || !network->initialized
             ? (pbns_pump_endpoint){0}
             : (pbns_pump_endpoint){
                   .read = tcp_read,
                   .write = tcp_write_endpoint,
                   .context = network,
               };
}
...
pbns_pico_network_attach_diagnostic(pbns_pico_network *network,
  ...
  if (network == NULL || !network->initialized || diagnostic == NULL) {
    return PBNS_ERR_ARGUMENT;
  }
  cyw43_arch_lwip_begin();
...
pbns_status pbns_pico_network_observe_diagnostic(
  ...
  cyw43_arch_lwip_begin();
  pbns_status status = PBNS_ERR_ARGUMENT;
  if (network->diagnostic == NULL) {
    cyw43_arch_lwip_end();
    return status;
  }
  switch (observation) {
  case PBNS_RAW_OBSERVE_NETWORK_INITIALIZED:
    status = pbns_raw_diagnostic_observe_milestone(
        network->diagnostic, PBNS_RAW_MILESTONE_NETWORK_INITIALIZED);
    break;
  case PBNS_RAW_OBSERVE_WIFI_STARTED:
    status = pbns_raw_diagnostic_observe_milestone(
        network->diagnostic, PBNS_RAW_MILESTONE_WIFI_STARTED);
    break;
  case PBNS_RAW_OBSERVE_WIFI_READY:
    status = pbns_raw_diagnostic_observe_milestone(
        network->diagnostic, PBNS_RAW_MILESTONE_WIFI_READY);
    break;
  case PBNS_RAW_OBSERVE_TCP_READY:
    status = pbns_raw_diagnostic_observe_milestone(
        network->diagnostic, PBNS_RAW_MILESTONE_TCP_READY);
    break;
  case PBNS_RAW_OBSERVE_CDC_RX:
    status = pbns_raw_diagnostic_observe_cdc_rx(network->diagnostic);
    break;
  case PBNS_RAW_OBSERVE_CDC_TX_ENQUEUE:
    status = pbns_raw_diagnostic_observe_cdc_tx_enqueue(network->diagnostic);
    break;
  case PBNS_RAW_OBSERVE_CDC_TX_FLUSH:
    status = pbns_raw_diagnostic_observe_cdc_tx_flush(network->diagnostic);
    break;
  case PBNS_RAW_OBSERVE_CDC_TX_COMPLETE:
    status = pbns_raw_diagnostic_observe_cdc_tx_complete(network->diagnostic);
    break;
  default:
    break;
  }
...
err_t pbns_pico_network_test_receive(pbns_pico_network *network,
                                     ...
                                     err_t error) {
  return tcp_received(network, pcb, packet, error);
}
#endif
#endif
